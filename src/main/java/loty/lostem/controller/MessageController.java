...
public class MessageController {
    ...
    @MessageMapping("/chat/message/lost") // setApplicationDestinationPrefixes()를 통해 prefix "/pub"으로 설정 했으므로, 경로가 한번 더 수정되어 “/pub/chat/message”로 바뀜
    ...
    @MessageMapping("/chat/message/found")
    public void foundMessage(@Payload ChatMessageDTO messageDTO, @Header("Authorization") String token) {
        tokenProvider.validateToken(token);
        log.info("유효한 토큰이므로 채팅을 계속합니다.");
        Long userId = tokenProvider.getUserId(token);

        chatService.createFoundMessage(messageDTO, userId);
        simpMessageSendingOperations.convertAndSend("/sub/chat/room/" + messageDTO.getRoomId(), messageDTO);
    }
